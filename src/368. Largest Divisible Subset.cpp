...
public:
    vector<int> largestDivisibleSubset(vector<int> &nums) {
        ...
        for (int n : nums) {
            ...
            for (int i = 0; i < N; i++) {
                auto &s = subsets[i];
                int last = s.nums[s.nums.size() - 1];
                if (n % last == 0) {
                    int f = n / last;
                    if (s.factors.count(f)) {
                        s.nums.push_back(n);
                    } else {
                        auto s1 = s;
                        s1.nums.push_back(n);
        ...
        sort(subsets.begin(), subsets.end(), [](const auto& l, const auto& r){
            return l.nums.size() > r.nums.size();
        });

        vector<int> r;
        r = subsets.size() ? subsets[0].nums : vector<int>{};
        if (has_one) { r.insert(r.begin(), 1); }
        return r;
    }
    ...
    vector<int> largestDivisibleSubset2(vector<int> &nums) {
        ...
        vector<int> r;
        while (max_set) {
            r.push_back(nums[max_i]);
            max_i = dp[max_i].second;
            max_set--;
        }
...
int main() {
    ...
    for (int i : Solution().largestDivisibleSubset2(nums)) {
        printf("%d ", i);
    }
    printf("\n");
    return 0;
