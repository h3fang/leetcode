...
impl Solution {
    pub fn min_skips(dist: Vec<i32>, speed: i32, hours_before: i32) -> i32 {
        let (speed, hours_before) = (speed as i64, hours_before as i64);
        if dist.iter().sum::<i32>() as i64 > speed * hours_before {
            return -1;
        }
        let n = dist.len();
        let mut f = vec![0; n + 1];
        for i in 0..n {
            ...
            for (j, &d) in dist[..n - 1].iter().enumerate() {
                let tmp = f[j + 1];
                f[j + 1] = ((f[j] + d as i64 + speed - 1) / speed) * speed;
                if i > 0 {
                    f[j + 1] = f[j + 1].min(pre + d as i64);
                }
                pre = tmp;
            }
            if f[n - 1] + dist[n - 1] as i64 <= speed * hours_before {
                return i as i32;
            }
        }
        -1
